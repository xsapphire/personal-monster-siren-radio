...
export const Player = (): ReactElement => { 
    ...
    useEffect(() => {
        const fetchSongDetail = async () => {
            const songDetail = await fetch(`/api/song/${myPlaylist[playingIndex-1].cid}`);
            const detail = await songDetail.json();
            
            setAlbumCoverUrl(allAlbums?.find(album => album.cid === detail.albumCid)?.coverUrl || '');
    ...
    return (
        <div style={{ width: "50%"}} >
            ...
            <FlexBox style={{padding: '16px'}} $center="xy" $direction="column">
                ...
                <h3>{playingIndex > 0 ? allSongs?.find((song: { cid: string; name: string }) => song.cid === myPlaylist[playingIndex-1].cid)?.name : 'None'}</h3>
